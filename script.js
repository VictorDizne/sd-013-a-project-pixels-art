// função para adicionar linhas e colunas no pixel-board!
const tbody = document.querySelector('#pixel-board tbody');
const trElem = document.getElementsByClassName('pixel');
const tdElem = document.querySelectorAll('.pixel td');

function addPixels(numCol = 5, numLin = 5) {
  for (let i = 0; i <= numCol - 1; i += 1) {
    const tr = document.createElement('tr');
    tr.className = 'pixel';
    tbody.appendChild(tr);
    for (let o = 0; o <= numLin - 1; o += 1) {
      const td = document.createElement('td');
      trElem[i].appendChild(td);
    }
  }
}

function addClass() {
  const item = document.querySelectorAll('.pixel td');
  for (i in item) {
    item[i].className = 'pixel';
  }
}
addPixels();
addClass();
console.log(tdElem);
