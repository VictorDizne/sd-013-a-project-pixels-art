...
function generateBoard(linesN) {
  const lines = document.getElementById('pixel-board');
  lines.innerHTML = '';
  for (let count1 = 0; count1 < linesN; count1 += 1) {
    const lines1 = lines.appendChild(document.createElement('div'));
    lines1.className = 'lines';
    for (let count2 = 0; count2 < linesN; count2 += 1) {
      const lines2 = lines1.appendChild(document.createElement('div'));
      lines2.className = 'pixel';
    }
  }
  for (let i = 0, len = pixel1.length; i < len; i += 1) {
...
function getColor(color) {
  ...
  const qr = document.querySelectorAll('.selected');
