...
function append(nodeElements) {
  ...
  return function whichChild(childElements) {
    const children = intoArray(childElements);
    return function whichElements(start, end) {
      if (!start) start = 0;
...
function clear(nodeElements) {
  const nodes = intoArray(nodeElements);
  return function () {
    for (let node of nodes) {
      while (node.children.length > 0) {
        let child = node.children[0];
        !isScript(child) && node.removeChild(child);
      }
    }
  };
}
...
function dom(selector, start, end) {
  ...
  self.clear = clear(this.elements);
